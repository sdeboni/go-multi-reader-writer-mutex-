...
func testWrite(t *testing.T, writer func(io.Writer) paasio.WriteCounter) {
...
func TestWriteReadWriter(t *testing.T) {
	testWrite(t, func(w io.Writer) paasio.WriteCounter {
		var r nopReader
...
func testRead(t *testing.T, reader func(io.Reader) paasio.ReadCounter) {
	chunkLen := 10 << 20 // 10MB
...
func TestReadReadWriter(t *testing.T) {
	testRead(t, func(r io.Reader) paasio.ReadCounter {
		var w nopWriter
		return NewReadWriteCounter(readWriter{r, w})
	})
}

func testReadTotal(t *testing.T, rc paasio.ReadCounter) {
	numGo := 8000
	numBytes := 50
...
func testWriteTotal(t *testing.T, wt paasio.WriteCounter) {
	numGo := 8000
	numBytes := 50
	totalBytes := int64(numGo) * int64(numBytes)
	p := make([]byte, numBytes)
...
func testReadCountConsistency(t *testing.T, rc paasio.ReadCounter) {
	const numGo = 4000
	const numBytes = 50
	p := make([]byte, numBytes)
...
func testWriteCountConsistency(t *testing.T, wc paasio.WriteCounter) {
	const numGo = 4000
	const numBytes = 50
	p := make([]byte, numBytes)
...
func BenchmarkReadMutex(b *testing.B) {
  testReadBench(b, mutex.NewReadCounter)
}
func BenchmarkReadRWMutex(b *testing.B) {
  testReadBench(b, rwmutex.NewReadCounter)
}
...
func BenchmarkReadValue(b *testing.B) {
  testReadBench(b, value.NewReadCounter)
}

func BenchmarkWriteMutex(b *testing.B) {
  testWriteBench(b, mutex.NewWriteCounter)
}
func BenchmarkWriteRWMutex(b *testing.B) {
  testWriteBench(b, rwmutex.NewWriteCounter)
}
...
func BenchmarkWriteValue(b *testing.B) {
  testWriteBench(b, value.NewWriteCounter)
}


func testReadBench(b *testing.B, reader func(io.Reader) paasio.ReadCounter) {
	numGo := 8000
	numBytes := 50
	p := make([]byte, numBytes)
...
func testWriteBench(b *testing.B, writer func(io.Writer) paasio.WriteCounter) {
	numGo := 8000
	numBytes := 50
	p := make([]byte, numBytes)
